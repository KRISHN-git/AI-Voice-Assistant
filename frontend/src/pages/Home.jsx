...
function Home() {
  ...
  useEffect(() => {
    ...
    recognition.onresult = async (e) => {
      ...
      if (transcript.toLowerCase().includes(userData.assistantName.toLowerCase())) {
        ...
        // Update local history so UI is consistent without needing a backend refresh
        setUserData(prev => ({
          ...prev,
    ...
    // Initial greeting is moved to the interaction overlay to satisfy browser autoplay policies
  ...
  return (
    <div className='w-full h-[100vh] bg-gradient-to-b from-[#361434] via-[#1d212c] to-[#1d212c] flex justify-center items-center flex-col gap-[20px] overflow-x-hidden'>
      ...
      {!hasInteracted && (
        <div
          className="absolute inset-0 z-[100] bg-[#1d212c]/90 backdrop-blur-md flex flex-col justify-center items-center cursor-pointer"
          onClick={() => {
            setHasInteracted(true);
            const greeting = window.SpeechSynthesisUtterance ? new SpeechSynthesisUtterance(`Hello ${userData.name}, what can I help you with?`) : null;
            if (greeting) {
              greeting.lang = 'hi-IN';
        ...
        >
          <div className="w-20 h-20 bg-cyan-500/20 rounded-full flex justify-center items-center animate-pulse mb-6">
            <div className="w-12 h-12 bg-cyan-400 rounded-full shadow-[0_0_20px_rgba(0,255,255,0.8)]"></div>
          </div>
          <h1 className="text-white text-3xl font-bold tracking-wider mb-2">Tap to Activate</h1>
          <p className="text-gray-300 text-sm">Your browser requires a tap to enable audio/voice output</p>
        </div>
      )}
  ...
  bg-white/20 backdrop-blur-xl border border-white/30 rounded-full cursor-pointer text-[19px] shadow-[0_0_15px_rgba(255,255,255,0.25)] hover:bg-white/30 transition'
      ...
      { }
      <div className={`w-[260px] h-[360px] flex justify-center items-center overflow-hidden rounded-3xl transition-all duration-300 ${aiText ? "shadow-[0_0_25px_rgba(0,255,255,0.45)] border border-teal-300" : "shadow-lg border border-transparent"
        }`}>
      ...
      <h1 className='text-white text-[18px] font-semibold text-center px-6 max-w-4xl leading-relaxed'>{userText ? userText : aiText ? aiText : null}</h1>
